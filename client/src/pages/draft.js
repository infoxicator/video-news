...
export default function Example() {
  ...
  return (
    <div className="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <button
        className="absolute top-0 right-0 m-5 bg-indigo-600 text-white px-1 py-1 rounded text-xl flex items-center space-x-1"
        onClick={onSubmit}
      >
      ...
      <div className="max-w-2xl w-full space-y-8">
        ...
        <div className="grid grid-cols-1 space-y-4">
          <div className="flex flex-col text-white">
            ...
            <input
              className="rounded bg-gray-600 text-2xl px-1 py-1"
              value={draftSubmission.title}
              onChange={e => onTextChange(e, "title")}
            />
            <label className="uppercase text-lg">Summary:</label>
            <textarea
              className="rounded bg-gray-600 text-2xl px-1 py-1 h-64"
              value={draftSubmission?.aiSummary?.split("\n").join("")}
              onChange={e => onTextChange(e, "aiSummary")}
            />
          </div>
          <div className="flex flex-wrap ">
            {draftSubmission.images &&
              draftSubmission.images.map((imageSrc, index) => (
