...
const LoadingSpinner = ({ text }) => (
  <div className="flex flex-col items-center justify-center py-16 md:py-48">
    ...
    >
      <LightningBoltIcon className="h-10 w-10 absolute top-1/2 left-0 -m-5 text-indigo-600" />
      {/* <Logo className="h-10 w-10 absolute bottom-0 left-1/2 -rotate-90 -m-5 opacity-75" /> */}
      <LightningBoltIcon className="h-10 w-10 absolute top-1/2 right-0 rotate-180 -m-5 opacity-50 text-indigo-600" />
      {/* <Logo className="h-10 w-10 absolute top-0 left-1/2 rotate-90 -m-5 opacity-25" /> */}
    </m.div>
    <p className="text-lg font-bold animate-pulse">{text}</p>
  </div>
)
